...
public class RecordEditBean {
    ...
    private Map<Integer, MultipleItemPair> pairBeanMap;//存储当前美妆选中的列表
    //捏脸数据
    ...
    public void setPairBeanMap(Map<Integer, MultipleItemPair> pairBeanMap) {
        this.pairBeanMap = new HashMap<>();
        for (Integer key : pairBeanMap.keySet()) {
            MultipleItemPair pairBean = new MultipleItemPair(pairBeanMap.get(key).getFrontLength(),
                                             pairBeanMap.get(key).getSelectItemPos(), pairBeanMap.get(key).getSelectColorPos());
            this.pairBeanMap.put(key, pairBean);
        }
    }

    public Map<Integer, MultipleItemPair> getPairBeanMap() {
        return pairBeanMap;
    }
