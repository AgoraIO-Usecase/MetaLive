...
import io.agora.metalive.manager.authpack;
import io.agora.metalive.manager.editface.constant.Constant;
import io.agora.metalive.manager.editface.constant.FUPTAClient;
...
public abstract class PTAClientWrapper {
    ...
    public static void initializeAvatarP2AData(@NonNull byte[] objData, @NonNull AvatarPTA avatarP2A) {
        ...
        avatarP2A.setHairIndex(FilePathFactory.getDefaultHairIndex(FilePathFactory.hairBundleRes(), hairLabel));
        int beardLabel = labels[1];
        avatarP2A.setBeardIndex(FilePathFactory.getDefaultIndex(FilePathFactory.beardBundleRes(), beardLabel));
        int hasGrasses = labels[2];
        int shapeGrasses = labels[3];
        int rimGrasses = labels[4];
        avatarP2A.setGlassesIndex(hasGrasses > 0 ? FilePathFactory.glassesIndex(shapeGrasses, rimGrasses) : 0);
        ...
        avatarP2A.setShoeIndex(Constant.style == Constant.style_art ? 0 : FilePathFactory.indexOfGender(FilePathFactory.shoeBundleRes(), avatarP2A.getGender()));
